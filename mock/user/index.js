import { getMenuListByRoleId, getAllMenuByRoleId, updateUserInfo } from '@/api/url';
// import { randomString } from '@/utils/utils';login
...
Mock.mock(RegExp(getMenuListByRoleId), function (options) {
  ...
  if (parseInt(roleId) === 1) { // 超级管理员
    return Mock.mock({ code: 200, data: adminRoutes, msg: '获取菜单列表成功', });
  } else if (parseInt(roleId) === 2) { // 编辑
    return Mock.mock({ code: 200, data: editorRoutes, msg: '获取菜单列表成功', });
  } else {
    return Mock.mock({ code: 500, data: '', msg: '目前仅支持超级管理员和编辑人员菜单', });
  }
});
